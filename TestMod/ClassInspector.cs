...
public static class ClassInspector
{
    ...
    public static void LogClassMembers(string name, Type type, int maxDepth = 2)
    {
        var types = new Dictionary<string, Type>();
        FindClassMembersRecursive(name, type, 0, maxDepth, types);
        //foreach (var kvp in types) {
        //    var memberType = kvp.Value;
        //    _logger?.Info.Log($"{kvp.Key} {memberType.FullName}");
        //}
        LogTypesAsWiki(types);
    }

    public static void LogTypesAsWiki(Dictionary<string, Type> types)
    {
        var sortedTypes = types.OrderBy(kvp => kvp.Key).ToList();

        StringBuilder sb = new StringBuilder();
        sb.AppendLine("{| class=\"wikitable sortable\"");
        sb.AppendLine("! Name !! Type !! Category");
        ...
        foreach (var kvp in sortedTypes)
        {
            ...
            // Type
            string type = GetSimpleTypeName(kvp.Value);
            sb.Append($"| <code>{type}</code> || ");

            // Category
            string category = GetTypeCategory(kvp.Value);
    ...
    private static void FindClassMembersRecursive(string name, Type type, int currentDepth, int maxDepth, Dictionary<string, Type> members)
    {
        if (type == null) { return; }
        ...
        {
            string newName = $"{name}.{field.Name}";
            FindClassMembersRecursive(newName, field.FieldType, currentDepth + 1, maxDepth, members);
        }
        ...
        foreach (PropertyInfo prop in properties)
        {
            ...
            FindClassMembersRecursive(newName, prop.PropertyType, currentDepth + 1, maxDepth, members);
        }
    }
    ...
    public static void LogDependencyContainerClasses(DependencyContainer container)
    {
        ...
        var types = new Dictionary<string, Type>();
        foreach (var kvp in container.BoundInstancesByResolveType)
        {
            types.Add(kvp.Key.Name, kvp.Value.GetType());
        }
    ...
    private static string GetSimpleTypeName(Type type)
    {
        if (type.IsGenericType)
        {
            string baseName = type.FullName.Split('`')[0];
